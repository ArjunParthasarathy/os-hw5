...
#endif /* CONFIG_SMP */
	/* no freezer tasks are queued on CPU runqueue */
...
/* put top element in freezer rq onto CPU runqueue associated w/ this runqueue */
...
dequeue_top_freezer_rq(struct freezer_rq *freezer_rq, unsigned int count)
{
	...
	/* element from freezer rq no longer queued on CPU runqueue */
	freezer_rq->freezer_queued = 0;
...
enqueue_top_freezer_rq(struct freezer_rq *freezer_rq)
{
	...
	/* we've already queued this entity */
	if (freezer_rq->freezer_queued)
		return;

	/* not worrying about throttling for this assignment */
	// if (freezer_rq_throttled(freezer_rq))
	// 	return;

	if (freezer_rq->freezer_nr_running) {
		add_nr_running(rq, freezer_rq->freezer_nr_running);
		/* the top element on freezer rq now queued on CPU rq */
...
// 		 */
// 		if (prio == prev_prio) {
// 			struct freezer_array *array = &freezer_rq->active;
...
void inc_freezer_tasks(struct sched_freezer_entity *freezer_se, struct freezer_rq *freezer_rq)
{
	...
	/* Increment number of tasks on the current freezer runqueue */
	freezer_rq->freezer_nr_running += 1;
...
static void __delist_freezer_entity(struct sched_freezer_entity *freezer_se, struct freezer_array *array)
{
	list_del_init(&freezer_se->run_list);

	/* For freezer we don't need to update the priority list at all*/
 ...
 * For freezer, we keep this logic unmodified but we need it to dequeue last entry
 */
...
static void dequeue_freezer_entity(struct sched_freezer_entity *freezer_se, unsigned int flags)
{
	...
	// This is no-op b/c group_freezer_rq is NULL
	// for_each_sched_freezer_entity(freezer_se) {
...
/*
 * Adding/removing a task to/from an array:
 */
static void
...
/*
 * Return the highest pushable other rq's task which is suitable to be executed
 * on this CPU's rq, NULL otherwise
 */
static struct task_struct *pick_highest_pushable_task_freezer(struct rq *rq, int cpu)
{
	...
	/* each entry has equal prio so this is just a loop */
...
static int find_lowest_rq_freezer(struct task_struct *task)
{
	...
	int i = 0;
	int min_cpu = cpu;
	int min_rq_len = task_rq(task)->freezer.freezer_rq_len;
...
static void switched_from_freezer(struct rq *rq, struct task_struct *p)
{
	 ...
	 * the balancing. But if we are the last RT task (and the CPU will go idle)
	 * we may need to handle the pulling of RT tasks (stealing from other CPU)
	 * now.
	 */
